# Ecosystem

Welcome to contribute to the ecosystem of this project! Below is a curated list of high-quality content for your reference and use. If you have excellent project-related tools or tutorials, feel free to share them by submitting a Discussion. For more details, please check out the [Ecosystem Call for Contributions](https://github.com/MarSeventh/CloudFlare-ImgBed/discussions/606).

## Plugins & Extensions

### Browser Extensions

#### Image Uploader to Markdown to CloudFlare-ImgBed
- **Author:** [calg_c](https://linux.do/u/calg_c/summary)
- **Link:** [Click here](https://greasyfork.org/zh-CN/scripts/529816-image-uploader-to-markdown-to-cloudflare-imgbed)
- **Description:** A browser extension powered by Tampermonkey that automatically uploads images to the image bed directly from within your editor.

#### Image/Video Uploader Helper
- **Author:** [yunskyline](https://github.com/yunskyline)
- **Link:** [Click here](https://greasyfork.org/en/scripts/552877-image-video-uploader-helper-intelligent-naming-version)
- **Description:** A powerful browser upload helper supporting smart naming and seamless one-click uploading for both images and videos.

### Blog & Note Plugins

#### CF-ImgBed-Uploader for Typecho
- **Author:** [Lord2333](https://github.com/Lord2333)
- **Link:** [Click here](https://github.com/Lord2333/CF-ImgBed-Uploader_for_Typecho)
- **Description:** A tailor-made image upload plugin for the Typecho blog system, providing a silky-smooth experience when writing posts.

#### obsidian-cf-imgbed
- **Author:** [fantasy-ke](https://github.com/fantasy-ke)
- **Link:** [Click here](https://github.com/fantasy-ke/obsidian-cf-imgbed/tree/1.0.2)
- **Description:** An exclusive plugin for Obsidian, allowing you to seamlessly connect to the cloud image bed while writing notes locally.

#### siyuan-cf-imgbed
- **Author:** [Sirin-Schariac](https://github.com/Sirin-Schariac)
- **Link:** [Click here](https://github.com/Sirin-Schariac/siyuan-cf-imgbed)
- **Description:** A SiYuan plugin that uploads local images to CloudFlare-ImgBed via context menus, paste, drag and drop, or batch operations, then automatically replaces them with remote images. It supports Telegram, Cloudflare R2, S3, Discord, Hugging Face, WebDAV, chunked uploads, and direct uploads to Hugging Face; settings and logs remain local.

#### wp-sanyueqicfimg
- **Author:** [imysen](https://github.com/imysen)
- **Link:** [Click here](https://github.com/imysen/wp-sanyueqicfimg)
- **Description:** A WordPress extension that easily replaces the local media library, drastically saving server storage space.

### System Drivers

#### OpenList Driver
- **Author:** [ZZ0YY](https://github.com/ZZ0YY)
- **Link:** [Click here](https://github.com/OpenListTeam/openlist)
- **Description:** Underlying driver support for the OpenList directory listing program, further expanding your storage boundaries.

## Peripheral Applications

### Clients & Bots

#### Satellite Desktop Client
- **Author:** **Official Development**
- **Link:** [Click here](https://github.com/MarSeventh/satellite)
- **Description:** The officially crafted desktop application, offering a highly intuitive and modern cross-platform interface for managing your image bed.

#### img-up-bot
- **Author:** [uki0xc](https://github.com/uki0xc)
- **Link:** [Click here](https://github.com/uki0xc/img-up-bot?tab=readme-ov-file)
- **Description:** A Telegram-based interactive bot. Just send an image in the chat to automatically upload it and receive the link.

### Auxiliary Tools

#### cloudflare-imgbed-auto-backup
- **Author:** [imysen](https://github.com/imysen)
- **Link:** [Click here](https://github.com/imysen/cloudflare-imgbed-auto-backup)
- **Description:** A practical set of automated metadata backup scripts, adding a hardcore "safety net" to your data.

### Derivative Projects

#### ImgBed
- **Author:** [axibayuit-a11y](https://github.com/axibayuit-a11y)
- **Link:** [Click here](https://md.ccwu.cc/)
- **Description:** A closed-source project refactored and integrated based on this repository. It supports 28 UI languages, 13 mainstream upload channels, backups, automatic tagging, access mode selection, user rate limiting, personal blogs, and magnet link uploads. The built-in toolkit features image editing, while file management and public image beds support switching between thumbnail and original image loading modes. With automated deployment, beginners can spin up their own site in just three minutes.

## AI Agents

### Agent Skills

#### cfbed-upload-skill
- **Author:** **Official Development**
- **Link:** [Click here](https://github.com/MarSeventh/cfbed-upload-skill)
- **Description:** The official AI agent skill plugin, empowering Large Language Models (LLMs) with the "superpower" to automatically transfer and process images.

## High-Quality Tutorials

### Video Tutorials

#### Official Deployment Tutorial
- **Author:** **Official Development**
- **Link:** [Click here](https://www.bilibili.com/video/BV1EeNu6zEAe/)
- **Description:** The authoritative, step-by-step official video guide, perfectly suited for absolute beginners.

### Text Tutorials

#### CloudFlare-ImgBed Advanced Tutorial Series
- **Author:** Yunsen's Cabin (Yunsen 的小窝)
- **Link:** [Click here](https://blog.imysen.com/category/cloudflare-imgbed/)
- **Description:** A hardcore, comprehensive guide from basic to advanced. Content covers private image bed setup, adult content detection configuration, domestic CDN multi-line routing optimization, and automated data backups via Github Actions.

#### Building a Telegram Channel Image Bed with Cloudflare Pages
- **Author:** lepidus.me
- **Link:** [Click here](https://blogstr.lepidus.me/post/1725801323700/)
- **Description:** An illustrated tinkering log that teaches you how to cleverly bridge Cloudflare Pages and Telegram channels to build a free pipeline.

#### Free Image Bed via CloudFlare and Telegram Tutorial
- **Author:** Liuxueguan | Blog (刘学馆 | Blog)
- **Link:** [Click here](https://blog.sexy0769.com/skill/735.html)
- **Description:** A practical, alternative approach detailing the zero-to-one parameter configuration and deployment process.

#### CloudFlare + Telegram: Build Your Own Free Image Bed
- **Author:** Datouding's little blog (大头钉的小小 blog)
- **Link:** [Click here](https://luckyting.top/index.php/archives/20/)
- **Description:** A detailed guide on how to leverage CloudFlare and Telegram to host your own free image bed.
