...
::: warning 注意
图片尺寸处理默认关闭，JPEG、PNG、WebP 和 AVIF 可在所有部署方式中处理；GIF 仅 Docker 部署可处理并保持 GIF 格式，Pages 和 Worker 部署默认返回 `415`；SVG 及其他格式不支持。Worker 和 Docker 的待处理源文件最大为 20 MB，Pages 以 Cloudflare Images 当前限制为准。使用 `fallback=original` 可在格式不支持、源文件超限或处理失败时返回原文件。使用前请在[配置说明 → 安全设置 → 访问管理](/deployment/configuration#访问管理)中开启功能并配置允许尺寸。
:::
