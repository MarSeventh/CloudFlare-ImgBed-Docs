...
- 读取文件接口新增 URL 图片尺寸处理，支持 `width`、`height`、`fit=cover` 居中裁剪和 `fit=squeeze` 拉伸；Cloudflare Worker 使用 Images binding，Docker 使用 Sharp
...
Documentation:
- 新增中英文批量删除 API 文档
- 新增中英文读取文件 API 文档，补充图片尺寸参数、部署差异和实际错误状态说明
...
- 精简前端状态管理，移除不再使用的 `credentials` 兼容映射
- 精简 Docker 运行时镜像内容，降低镜像体积
...
Add Features:
- 系统设置的公告配置新增“刷新公告已读状态”选项；公告内容发生变化或管理员手动刷新后，已确认用户下次访问上传页时会再次看到公告
- 上传页公告改为用户点击“我已知晓”后记录确认时间和公告版本，并兼容旧版首次访问标记

Security:

- 增强fetchRes接口的目标URL合法性检测
- 升级依赖，提升安全性
